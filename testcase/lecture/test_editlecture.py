...
class TestEditlecture(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.data = todict.data(os.path.join(os.path.dirname(__file__), '../../data', 'lecture_editlecture.json'))
        cls.data['request']['headers']['Cookie'] = cookie
        cls.data['request']['json']['productCode'] = randdata.getproductline()

    def test_editlecture(self):
        global res
        try:
            ...
            log.info(f'lecture/test_editlecture: FAIL, error is {error}')
        finally:
            self.assertEqual(200, res.status_code)
            log.info('lecture/test_editlecture: SUCCESS！')
